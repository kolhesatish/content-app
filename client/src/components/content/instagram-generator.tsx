...
import { generateInstagramContent } from "@/lib/content-api";

type ContentType = "post" | "reel" | "story";
type CaptionPreference = "yes" | "no";
...
export default function InstagramGenerator() {
  ...
  const generateMutation = useMutation({
    mutationFn: generateInstagramContent,
    onSuccess: (data) => {
      ...
      {currentStep === 1 && (
        <div className="glass-card rounded-2xl p-8">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            {[
              { type: "post" as ContentType, icon: Image, title: "Post", description: "Static image posts with captions" },
              { type: "reel" as ContentType, icon: Video, title: "Reel", description: "Short-form video content" },
              { type: "story" as ContentType, icon: Zap, title: "Story", description: "24-hour disappearing content" }
          ...
          <div className="space-y-6 mb-8">
            <div>
              ...
              <div className="flex gap-4">
                {[
                  { value: "yes" as CaptionPreference, label: "Yes, generate a caption" },
                  { value: "no" as CaptionPreference, label: "No caption needed" }
                ].map(({ value, label }) => (
                  <button
                    key={value}
