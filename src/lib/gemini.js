...
export function createInstagramPrompt(topic, contentType, options) {
  ...
  if (contentType === 'post') {
    prompt += ` Create Instagram post captions that users can copy and paste directly. Each variation should include a captivating caption (approximately ${wordCount} words) that encourages engagement, relevant hashtags (8-12) for better reach, and a specific content style approach. Make the captions ready to use for Instagram posts. `;
  } else if (contentType === 'reel') {
    prompt += ` Create Instagram reel content that users can use to create engaging videos. Each variation should include an attention-grabbing hook for the first 3 seconds, a detailed script outline (approximately ${wordCount} words) for 15-30 seconds of video content, a clear call-to-action to boost engagement, and trending hashtags for maximum reach. `;
  } else if (contentType === 'story') {
    prompt += ` Create Instagram story content that users can use for their 24-hour stories. Each variation should include engaging story text (approximately ${wordCount} words) that works well as overlay text, interactive elements suggestions, and creative ideas. Do NOT include hashtags for stories as they're not commonly used. `;
  }
